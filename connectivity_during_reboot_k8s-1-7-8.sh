...
for worker_node in $worker_nodes; do
  ...
  while [ "$count" -lt "$iter" ]; do
    ...
    if [ "$cnodes_current" -eq "$(cnodes_avail)" ]; then
      ...
      if [ "$(cnode_term)" -ne "0" ]; then 
        echo "$(kubectl get ds -n kube-system calico-node)"
        echo "echoing state if Terminating  $(kubectl get pods -n kube-system -l k8s-app=calico-node -o wide | grep -v "Terminating"| grep ${worker_node})"
